    ...
    <nav className="fixed bottom-0 left-0 right-0 z-40 lg:hidden" style={{background:'var(--color-surface)', borderTop:'1px solid var(--color-border)', padding:'6px 0 calc(6px + env(safe-area-inset-bottom))'}}>
      <div className="flex items-center">
        {([
          ...
          return (
            <Link key={item.href} href={item.href} className="relative flex-1 flex flex-col items-center gap-[3px] py-[6px] px-1" style={{color: isActive ? 'var(--color-brand)' : 'var(--color-muted)', fontSize:'10.5px', fontWeight: isActive ? 600 : 500, fontFamily:'var(--font-body)'}}>
              <FontAwesomeIcon icon={item.icon} className="w-5 h-5" />
              <span>{item.label}</span>
              {isActive && <span className="absolute top-0 left-1/2 -translate-x-1/2 w-6 rounded-full" style={{height:'2px', background:'var(--color-gold)'}} />}
            </Link>
          );
        })}
        ...
        <div className="flex-none relative" style={{width:'56px'}}>
          <button
            onClick={() => {
              ...
              const createRoute = CREATE_ROUTES.find(r => pathname.startsWith(r));
        ...
        {(() => {
          ...
          return (
            <Link href={fourthSlot.href} className="relative flex-1 flex flex-col items-center gap-[3px] py-[6px] px-1" style={{color: isActive ? 'var(--color-brand)' : 'var(--color-muted)', fontSize:'10.5px', fontWeight: isActive ? 600 : 500, fontFamily:'var(--font-body)'}}>
