...
import {
  ...
  faTriangleExclamation, faLock, faRightFromBracket, faChevronRight
    ...
    <div className="max-w-2xl mx-auto pb-8">
      ...
      <div className="rounded-2xl overflow-hidden mb-6" style={{ background: "var(--color-surface)", border: "1px solid var(--color-border)" }}>
        {MENU_ITEMS.map((item, i) => (
          <Link
            key={item.key}
            href={item.href}
            className="flex items-center gap-4 px-4 py-4 transition-colors hover:bg-[var(--color-brand-subtle)]"
      ...
      >
        <Link
          ...
          className="flex items-center gap-4 px-4 py-4 transition-colors hover:bg-red-50"
        >
          <div
            className="w-10 h-10 rounded-full flex items-center justify-center shrink-0"
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs mt-0.5" style={{ color: "#94A3B8" }}>Désactiver ou supprimer votre compte</p>
          </div>
          <FontAwesomeIcon icon={faChevronRight} className="w-4 h-4 shrink-0" style={{ color: "#94A3B8" }} />
        </Link>
      </div>
