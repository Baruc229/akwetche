...
import { faBox, faPlus, faPen, faTrash, faArrowTrendUp, faXmark, faSearch, faSort } from '@fortawesome/free-solid-svg-icons';
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <div className="relative">
              <button
                ...
                className="input-field flex items-center gap-2 cursor-pointer min-w-[140px]"
              >
                <FontAwesomeIcon icon={faSort} className="w-4 h-4 text-muted" />
                <span className="flex-1 text-left text-sm">{sortLabel}</span>
              </button>
              {showSortMenu && (
                <div className="absolute z-30 right-0 mt-1 bg-white border border-border rounded-xl shadow-lg overflow-hidden animate-fade-in min-w-[180px]">
                  {(["name", "salePrice", "margin", "stock"] as SortKey[]).map((key) => (
                    <button
                      key={key}
                      type="button"
          ...
          {sorted.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {sorted.map((p) => {
                ...
                const marginRate = p.salePrice > 0 ? ((margin / p.salePrice) * 100).toFixed(0) : "0";
                return (
                    ...
                    <div className="space-y-1.5 text-sm">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium text-forest">
                          <FontAwesomeIcon icon={faArrowTrendUp} className="w-3 h-3 mr-1 inline" />
